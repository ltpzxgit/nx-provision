...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($os) {
    $stmt = $pdo->prepare("INSERT INTO os_support (os, new_provision, deep, zabbix_agent, crowdstrike, template, other, eos, std_nonstd)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([$os, $new_provision, $deep, $zabbix_agent, $crowdstrike, $template, $other, $eos, $std_nonstd]);
